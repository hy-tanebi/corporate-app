...
このファイルは、AIコーディングアシスタントClaudeが当プロジェクトで開発支援を行う際の、基本原則とルールを定めたものである。Claudeは、全ての応答とコード生成において、このファイルの内容を最優先事項として遵守すること。また、必ず日本語で説明・応答するようにしてください。特に、実装計画（Implementation Plan）やタスクリストの作成・説明においては、必ず日本語を使用すること。
