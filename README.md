...
- **液体エフェクトの描画を 2 パス化**: 合成用の全画面メッシュがメインのシーンに置かれていたため、R3F が毎フレーム canvas へ描いていたシーンが、直後にこのメッシュで覆われて丸ごと捨てられていた。メッシュを合成専用シーンへ分離し、`useFrame` の `renderPriority` で自動描画を止めて「シーン→FBO」「FBO→canvas」の 2 パスに固定。canvas へ描くのは全画面メッシュ 1 枚だけになり、無駄なシーン描画が消えた

本番実測（Lighthouse モバイル）: スコア 46 → 74、LCP 20.2 秒 → 4.5 秒、TBT 1,710ms → 130ms、総転送量 3.99MB → 1.79MB。

なおこの計測はモバイル設定であり、液体エフェクトはモバイルでは描画していません。上記のうち 2 パス化の効果はこの数値には含まれていません。

### 描画パイプライン（デスクトップの液体エフェクト）

トップページの液体エフェクトは、シーンの一部ではなく**合成パス**です。FBO に撮ったシーンをシェーダーで歪ませ、`NoBlending` の全画面メッシュとして出力しています。つまりデスクトップで見えている絵は、シーンそのものではなく FBO のキャプチャです。

当初はこの合成メッシュがメインのシーンに置かれていたため、R3F が毎フレーム canvas へ描いたシーンが、直後にこのメッシュで完全に覆われて捨てられていました。合成メッシュを専用シーンへ分離し、`useFrame` に正の `renderPriority` を渡して R3F の自動描画を止めることで、描画を 2 パスに固定しています。

```
パス1: シーン → FBO
パス2: FBO   → canvas（全画面メッシュ 1 枚だけ）
```

この構造では **FBO の更新頻度がそのまま画面内容の更新頻度**になります。負荷軽減のつもりで FBO の更新を間引くと、ブラウザが報告するフレームレートは 60fps のまま（合成メッシュ自体は毎フレーム描き直されるため）で、シーンの動きだけが目に見えて粗くなります。一度この判断を誤ってスクロール演出を 1/3 の更新頻度にしてしまい、元に戻した経緯があります。
