...
const menus: Menu[] = [
	{
		...
		title: "Webサイトの制作・改善",
		body: [
			"問い合わせが来ないとき、原因は見た目より前にあることがほとんどです。何の会社か分かるまでに時間がかかる、知りたいことが書いていない、問い合わせ先が見つからない、フォームの項目が多すぎる。人はそこで黙って離れていきます。まず、どこで落ちているかを見るところから始めます。",
			"見るのは、どのページがよく見られていて、どこで離れているか。スマホでどう表示されるか。表示までどれくらい待たされるか。GA4が入っていれば数字も一緒に見ますし、入っていなければ入れるところからです。直す順番は効きそうなところから決めるので、全ページをつくり直す必要はありません。これから新しくつくる場合も、何を載せるかの整理から一緒に進めます。",
			"そして、問い合わせが増えても、受ける側が追いつかなければ売上には変わりません。誰に届いて、どう返して、どこに記録が残るか。ここが手作業のままだと、来たはずの話が止まります。サイトと社内の仕組みを、分けて考えません。入口から次の連絡までを一続きで設計します。サイトをつくる会社と社内のツールをつくる会社が別々だと、ここが宙に浮きがちな部分です。",
		...
		items: [
			"問い合わせが落ちている箇所の洗い出し（導線・フォーム・スマホ表示・ページ速度・GA4）",
